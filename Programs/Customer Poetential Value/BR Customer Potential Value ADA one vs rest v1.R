...
headers1<-cbind("eventcat", "run", "nu", "TPR","TNR", "Accuracy", "AUC","Concordance")
...
headers2<-cbind("eventcat", "run", "nu", "SampleNumber",
...
for (s in 1:5)
{
  ...
  for(j in 1:length(c.list))
  {
    ...
    ada.model <- ada(pv_segment_p1 ~., data=tra.samp, type="discrete", 
					 ...
					 nu=c.list[j])
    save(ada.model, file = paste0("BR/RData/ADA/br_",eventcat,'_vs_rest',
                                   "_",z,".RData"))
