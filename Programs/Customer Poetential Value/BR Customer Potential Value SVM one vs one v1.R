...
headers1<-cbind("eventcat", "refcat", "run", "sigma","cost", "trainingerror","CVerror","SV/TotalObs",
...
headers2<-cbind("eventcat", "refcat", "run", "sigma","cost","SampleNumber",
                "TPR","TNR", "Accuracy", "AUC","Concordance")
...
for (s in 1:5)
{
  ...
  for(j in 1:length(c.list))
  {
    ...
    save(ksvm.model, file = paste0("BR/RData/br_",eventcat,'_vs_',
                                   refcat,"_",z,".RData"))
    ...
    print(paste(eventcat, refcat, s, c.list[j], tpr, tnr, 
                acc, auc, concordance))
    
    write.table(cbind(eventcat, refcat, s, sigma, c.list[j],
                      attr(ksvm.model,"error"),attr(ksvm.model,"cross"),
                      nSV(ksvm.model)/10000, tpr, tnr, acc, auc, 
                      concordance), 
    ...
    for (l in 1:5)
    {
      #tst.br.mod <- tst.br[(tst.br[,2]==refcat | tst.br[,2] == eventcat), ]
      tst.br.mod <- tst.br
      ...
      print(paste(eventcat, refcat, s, c.list[j], l, tpr.tst, tnr.tst,
                  acc.tst, auc.tst, concordance.tst))
      
      write.table(cbind(eventcat, refcat, s, sigma, c.list[j], l, 
                        tpr.tst, tnr.tst, acc.tst, auc.tst, 
                        concordance.tst),
