...
PROC UNIVARIATE DATA=DATALIB.GO_ALLCAMPAIGNS_RESPONDERS;
VAR NET_SALES_AMT;
OUTPUT OUT=DATALIB.GO_NET_SLS_PROMO_PCTL
	   PCTLPRE = NET_SALES_AMT_
	   PCTLPTS= 0.5 50 99.5;
RUN;

DATA GO_ALLCAMPAIGNS_RESPONDERS_1;
KEEP NET_SALES_AMT AVG_ORD_SZ_12MO_CP ONSALE_QTY_12MO AVG_UNT_RTL_CP
	 UNITS_PER_TXN_12MO_SLS DIV_SHP_GO TREATMENT_GROUP RESPONDER
	 ONSALE_QTY_6MO DAYS_LAST_PUR_GO AVG_ORD_SZ_12MO
	 NET_SALES_AMT_12MO_RTN AVG_UNT_RTL NUM_TXNS_12MO_SLS GO_PROMOTIONS_RECEIVED_12MO
...
DATA DATALIB.GO_NET_SLS_PROMO_PCTL; 
 SET DATALIB.GO_NET_SLS_PROMO_PCTL;
...
DATA GO_ALLCAMPAIGNS_RESPONDERS_2;
SET GO_ALLCAMPAIGNS_RESPONDERS_1;
...
DATA GO_ALLCAMPAIGNS_RESPONDERS_3;
SET GO_ALLCAMPAIGNS_RESPONDERS_2;
...
PROC REG DATA=GO_ALLCAMPAIGNS_RESPONDERS_3 OUTEST=DATALIB.GO_ALLCAMPAIGNS_REVEST;
 MODEL NET_SALES_AMT =  	  AVG_ORD_SZ_12MO
...
PROC REG DATA=DATALIB.GO_BALANCED_ORIG_SCORES;
...
PROC SCORE DATA=DATALIB.GO_ALLCAMPAIGNS_BALANCED_FITS 
		   OUT=DATALIB.GO_ALLCAMPAIGNS_BALANCED_FITS
		   SCORE=DATALIB.GO_ALLCAMPAIGNS_REVEST TYPE=PARMS;
 VAR AVG_ORD_SZ_12MO
	 NET_SALES_AMT_12MO_SLS 
	 AVG_UNT_RTL 
	 DISCOUNT_PCT_12MO 
...
PROC LOGISTIC DATA=DATALIB.GO_BALANCED_ORIG_SCORES;
  MODEL RESPONDER(EVENT="1")=	EXP_PROF
								/EXPB LACKFIT CTABLE;
RUN;

PROC SQL;
 CREATE TABLE DATALIB.GO_BALANCED_VALIDATION AS SELECT T1.*, T2.POFFSET, T2.NEW_EXP_REVENUE, T2.NEW_PREDICTED_REVENUE 
...
PROC SQL;
 SELECT COUNT(*) INTO: TRAINCOUNT FROM DATALIB.GO_BALANCED_ORIG_SCORES;
QUIT;
 
%LET TRAININTV = %EVAL(&TRAINCOUNT/10);

PROC SORT DATA=DATALIB.GO_BALANCED_ORIG_SCORES;
 BY DESCENDING EXP_PROF DESCENDING NET_SALES_AMT;
RUN;

DATA DATALIB.GO_BALANCED_ORIG_SCORES;
 SET DATALIB.GO_BALANCED_ORIG_SCORES;
...
PROC SQL;
 CREATE TABLE TEST1 AS SELECT DISTINCT EXP_PROF_GRP, COUNT(CUSTOMER_KEY) AS CUSTOMERS,
	    ...
	    FROM DATALIB.GO_BALANCED_ORIG_SCORES
...
PROC SQL;
 SELECT COUNT(*) INTO: TRAINCOUNT FROM DATALIB.GO_ALLCAMPAIGNS_BALANCED_FITS;
QUIT;
 
%LET TRAININTV = %EVAL(&TRAINCOUNT/10);

PROC SORT DATA=DATALIB.GO_ALLCAMPAIGNS_BALANCED_FITS;
...
PROC FREQ DATA=DATALIB.GO_ALLCAMPAIGNS_BALANCED_FITS;
...
PROC SQL;
 CREATE TABLE TEST2 AS SELECT DISTINCT NEW_EXP_REVENUE_GRP, COUNT(CUSTOMER_KEY) AS CUSTOMERS,
 									   SUM(RESPONDER) AS RESPONDERS, SUM(NET_SALES_AMT) AS NET_SALES
	    FROM DATALIB.GO_ALLCAMPAIGNS_BALANCED_FITS
