...
modelfilepath <- '//10.8.8.51/lv0/Tanumoy/Datasets/Parameter Files/AT Catalog Response/MLP/v4/'

modellist <- list.files(path=modelfilepath, full.names=T, pattern='.params')  


j <- 1
for (j in 1:length(modellist))
{
  modellist[j] = ifelse(regexpr('.params', modellist[j])>0, 
                        gsub('.params', '', modellist[j]), modellist[j])
}  
...
for (j in 1:length(modellist))
{
  ...
  rounds <- as.numeric(substr(modellist[j], regexpr('-', modellist[j]) + 1, nchar(modellist[j])))
  model.name <- substr(modellist[j], 1, regexpr('-', modellist[j]) - 1)
    
  mlp.model <- mx.model.load(model.name, rounds)
  
  at.data.shaped <- at.data[,c(-1, -ncol(at.data))]
  
  prop <- sum(at.data.shaped[,1])/nrow(at.data.shaped)
  
  at.data.shaped <- data.matrix(at.data.shaped)
  
  at.data.shaped.x <- at.data.shaped[,-1]
  at.data.shaped.y <- at.data.shaped[, 1]
  
  preds <- predict(mlp.model, at.data.shaped.x, array.layout = 'rowmajor')
  prob_tst <- (t(preds))[,2]
...
write_csv(data.frame(aucmatrix), "//10.8.8.51/lv0/Tanumoy/Datasets/Model Results/Catalog Response Model/at_catalog_fall_validation_mlp.csv", col_names=T)
