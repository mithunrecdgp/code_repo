...
## Start a local cluster with 14GB RAM
h2o.init(ip = "localhost", port = 54321, startH2O = T, 
         max_mem_size = '14G', nthreads=-1)
...
modelfilepath <- '//10.8.8.51/lv0/Tanumoy/Datasets/Parameter Files/AT Catalog Response/H2O/v4/'
...
for (j in 1:length(modellist))
{
  ...
  h2o.model <- h2o.loadModel(path=paste0(modellist[j], '/', list.files(modellist[j])))
  
  prop.tst <- sum(at.data[,2])/nrow(at.data)
  
  at.data.h2o <- as.h2o(x=at.data[,3:(ncol(at.data)-1)], destination_frame='h2o.data.frame')
  
  prob_tst <- matrix(as.numeric(h2o.predict(h2o.model, at.data.h2o)), 
                     nrow(at.data.h2o), 3)[,3]
...
write_csv(data.frame(aucmatrix), "//10.8.8.51/lv0/Tanumoy/Datasets/Model Results/Catalog Response Model/at_catalog_fall_validation_h2o.csv", col_names=T)
