...
modelfilepath <- '//10.8.8.51/lv0/Tanumoy/Datasets/Parameter Files/AT Catalog Response/GBM/v4/'
...
for (j in 1:length(modellist))
{
  ...
  prob_tst <- predict.gbm(object=gbm.model, data=data.frame(at.data[,3:(ncol(at.data)-1)]), type='response', n.trees=opt.num.trees)
...
write_csv(data.frame(aucmatrix), "//10.8.8.51/lv0/Tanumoy/Datasets/Model Results/Catalog Response Model/at_catalog_fall_validation_gbm.csv", col_names=T)
