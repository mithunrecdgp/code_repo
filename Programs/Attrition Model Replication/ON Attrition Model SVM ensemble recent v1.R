...
on.data <-read.csv.ffdf(file="//10.8.8.51/lv0/Saumya/Datasets/on_recent_data.csv",header = TRUE,VERBOSE = TRUE,
                        sep=',',colClasses = c(rep("numeric",14)))

on.data[,2]<-1-on.data[,2]


runs  <- (nrow(on.data) - mod(nrow(on.data), sample.size))/sample.size
...
write.table(cbind("Testing_No","Customer_Key","Actual_response",
            ...
            '//10.8.8.51/lv0/Saumya/Datasets/on_recent_ensemble_testing.csv',
            append=FALSE,sep=",",row.names=FALSE,col.names=FALSE)

bigindex<-sample(1:nrow(on.data), size=sample.size * runs)

for (j in 1:runs) 
{
    print('-----------------------------------------------')
    index <- bigindex[((j-1)*sample.size+1):(j*sample.size)]
    tst.on<-on.data[index,]
    #tst.on<-tst.on[c(-1)]
    ...
    for (i in 1:20 )
    {
          ...
          load(paste0("//10.8.8.51/lv0/Saumya/Datasets/SVM_Models/ON/Recent/","model",i,".RData"))
          prob_tst<- predict(ksvm.model,tst.on[c(-1)],type="probabilities")
          
          if(i==1) 
          {
            output <- cbind(rep(j,sample.size), tst.on[,1], tst.on[,2], prob_tst[,2])
            rm(prob_tst)
            gc()
          }
    ...
    write.table(output,'//10.8.8.51/lv0/Saumya/Datasets/on_recent_ensemble_testing.csv', append=TRUE,sep=",",row.names=FALSE,col.names=FALSE)
