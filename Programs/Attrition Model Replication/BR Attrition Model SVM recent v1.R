...
br.data <-read.csv.ffdf(file="//10.8.8.51/lv0/Saumya/Datasets/br_recent_data.csv",
                        header = TRUE,VERBOSE = TRUE,
                        sep=',',colClasses = c(rep("numeric",16)))

br.data[,2]<-1-br.data[,2]

br.data <- subset(br.data, frequency >= 1)
...
write.table(headers1, paste0('//10.8.8.51/lv0/Saumya/Datasets/br_training_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)

headers2<-cbind("samplesize","run", "sigma","cost","SampleNumber","AUC","Concordance")
write.table(headers2, paste0('//10.8.8.51/lv0/Saumya/Datasets/br_testing_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)


interrupted.sample.size <- 5000;

interrupted.run <- 1; interrupted.c <- 0.1;
...
{ 
  ...
  for (s in startindex:10)
  {
    ...
    for(j in 1:length(start.c.list))
    {
      ...
      ksvm.model <- ksvm(
        as.factor(flag_2013) ~., data=tra.br, type='C-svc',
      ...
      write.table(cbind(sample.sizes[i], s, sigma,start.c.list[j],attr(ksvm.model,"error"),attr(ksvm.model,"cross"),nSV(ksvm.model)/sample.sizes[i],roc.area,concordance),
                  paste0('//10.8.8.51/lv0/Saumya/Datasets/br_training_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
      ...
      for (l in 1:10)
      {
        ...
        write.table(cbind(sample.sizes[i], s, sigma,start.c.list[j],l,roc.area.tst,concordance.tst),
                    paste0('//10.8.8.51/lv0/Saumya/Datasets/br_testing_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
                    append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)      
      }
...
model <- read.csv(file="//10.8.8.51/lv0/Saumya/Datasets/br_model_ensemble_recent.csv",head=TRUE,sep=",")

headers1 <- cbind("samplesize", "run", "sigma","cost","trainingerror","CVerror","SV_TotalObs",
                  "AUC","Concordance", "Test_AUC", "Test_Concordance")
write.table(headers1, paste0('//10.8.8.51/lv0/Saumya/Datasets/br_model_results_ensemble_recent.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)
...
for (i in 1:nrow(model))
{
  ...
  for (s in 1:10)
  {
    ...
    br.data.ones <- br.data[br.data[,2]==1,]
    index.ones.tra <- bigsample(1:nrow(br.data.ones), size=0.5*sample.size, replace=F)
    tra.br.ones <- br.data.ones[ index.ones.tra,]
    tst.br.ones <- br.data.ones[-index.ones.tra,]
    ...
    br.data.zeroes <- br.data[br.data[,2]!=1,]
    index.zeroes.tra <- bigsample(1:nrow(br.data.zeroes), size=0.5*sample.size, replace=F)
    tra.br.zeroes <- br.data.zeroes[ index.zeroes.tra,]
    tst.br.zeroes <- br.data.zeroes[-index.zeroes.tra,]
    ...
    for (l in 1:20)
    {
      ...
      tst.br <- rbind(tst.br.ones, tst.br.zeroes)[index.tst.br, ]
      tst.br<-tst.br[c(-1)]
      gc()
      prob_tst<- predict(ksvm.model,tst.br[c(-1)],type="probabilities")
    ...
    write.table(cbind(sample.size, s, sigma, cost, attr(ksvm.model,"error"), 
                ...
                paste0('//10.8.8.51/lv0/Saumya/Datasets/br_model_results_ensemble_recent.csv'),
                append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)
    ...
    if (roc.area.tst >= test.auc & concordance.tst >= test.concordance)
    {
      print('-------------------- Saving Model -------------------------')
      print(ksvm.object)
      save(ksvm.model, file = paste0("//10.8.8.51/lv0/Saumya/Datasets/SVM_Models/BR/Recent/","model",i,".RData"))
