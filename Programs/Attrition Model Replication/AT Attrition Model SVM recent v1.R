...
write.table(headers1, paste0('//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/at_training_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)

headers2<-cbind("samplesize","run", "sigma","cost","SampleNumber","AUC","Concordance")
write.table(headers2, paste0('//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/at_testing_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)
  ...
  for (s in 1:10)
  {
    ...
    for(j in 1:length(c.list))
    {
      ...
      write.table(cbind(sample.sizes[i], s, sigma,c.list[j],attr(ksvm.model,"error"),attr(ksvm.model,"cross"),nSV(ksvm.model)/sample.sizes[i],roc.area,concordance),
                  paste0('//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/at_training_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
      ...
      for (l in 1:10)
      {
        ...
        write.table(cbind(sample.sizes[i], s, sigma,c.list[j],l,roc.area.tst,concordance.tst),
                    paste0('//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/at_testing_recent_', min.sample.size,'_', max.sample.size, '.csv'), 
                    append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)      
      }
...
model <- read.csv(file="//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/at_model_ensemble_recent.csv",head=TRUE,sep=",")

headers1 <- cbind("samplesize", "run", "sigma","cost","trainingerror","CVerror","SV_TotalObs",
                  "AUC","Concordance", "Test_AUC", "Test_Concordance")
write.table(headers1, paste0('//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/at_model_results_ensemble_recent.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)
...
for (i in 1:nrow(model))
{
  ...
  for (s in 1:10)
  {
    ...
    write.table(cbind(sample.size, s, sigma, cost, attr(ksvm.model,"error"), 
                ...
                paste0('//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/at_model_results_ensemble_recent.csv'),
                append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)
    ...
    if (roc.area.tst >= test.auc & concordance.tst >= test.concordance)
    {
      print('-------------------- Saving Model -------------------------')
      print(ksvm.object)
      save(ksvm.model, file = paste0("//10.8.8.51/lv0/Tanumoy/Datasets/SVM_Models/AT/Recent/","model",i,".RData"))
