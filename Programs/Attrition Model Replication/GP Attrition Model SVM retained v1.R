...
gp.data <-read.csv.ffdf(file="//10.8.8.51/lv0/Saumya/Datasets/gp_final_ratio_flag_rec.csv",header = TRUE,VERBOSE = TRUE,
...
write.table(headers1, paste0('//10.8.8.51/lv0/Saumya/Datasets/gp_training_retained_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)

headers2<-cbind("samplesize","run", "sigma","cost","SampleNumber","AUC","Concordance")
write.table(headers2, paste0('//10.8.8.51/lv0/Saumya/Datasets/gp_testing_retained_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)
...
{ 
  for (s in 1:10)
  {
    ...
    tra.gp.ones <- gp.data.ones[ index.ones.tra,]
    tst.gp.ones <- gp.data.ones[-index.ones.tra,]
    ...
    tra.gp.zeroes <- gp.data.zeroes[ index.zeroes.tra,]
    tst.gp.zeroes <- gp.data.zeroes[-index.zeroes.tra,]
    ...
    for(j in 1:length(c.list))
    {
      ...
      ksvm.model <- ksvm(
        as.factor(flag_2013) ~., data=tra.gp, type='C-svc',
      ...
      prob_tra<- predict(ksvm.model,tra.gp[c(-1)],type="probabilities")
      
      
      x<-matrix(prob_tra[,2], nrow(tra.gp), 1)
      y<-matrix(tra.gp[,1], nrow(tra.gp), 1)
      z<-cbind(x,y)
      zeroes <- z[ which(y==0), ]
      ones <- z[ which(y==1), ]
      loopi<-nrow(ones)
      ...
      for (p in 1:nrow(roc.grid))
      {
        ...
        roc.grid[p,1] = cutoff
        roc.grid[p,2] = sum(prob_tra[,2]>cutoff & tra.gp[,1]==1)   # true positives
        roc.grid[p,3] = sum(prob_tra[,2]<=cutoff & tra.gp[,1]==0)  # true negatives
        roc.grid[p,4] = sum(prob_tra[,2]>cutoff & tra.gp[,1]==0)   # false positives
        roc.grid[p,5] = sum(prob_tra[,2]<=cutoff & tra.gp[,1]==1)  # false negatives
      ...
      write.table(cbind(sample.sizes[i], s, sigma,c.list[j],attr(ksvm.model,"error"),attr(ksvm.model,"cross"),nSV(ksvm.model)/sample.sizes[i],roc.area,concordance),
                  paste0('//10.8.8.51/lv0/Saumya/Datasets/gp_training_retained_', min.sample.size,'_', max.sample.size, '.csv'), 
      ...
      for (l in 1:10)
      {
        ...
        tst.gp<-tst.gp[c(-1)]
        gc()
        prob_tst<- predict(ksvm.model,tst.gp[c(-1)],type="probabilities")
        
        x<-matrix(prob_tst[,2], nrow(tst.gp), 1)
        y<-matrix(tst.gp[,1], nrow(tst.gp), 1)
        z<-cbind(x,y)
        zeroes <- z[ which(y==0), ]
        ones <- z[ which(y==1), ]
        loopi<-nrow(ones)
        ...
        for (p in 1:nrow(roc.grid))
        {
          ...
          roc.grid[p,1] = cutoff
          roc.grid[p,2] = sum(prob_tst[,2]>cutoff & tst.gp[,1]==1)   # true positives
          roc.grid[p,3] = sum(prob_tst[,2]<=cutoff & tst.gp[,1]==0)  # true negatives
          roc.grid[p,4] = sum(prob_tst[,2]>cutoff & tst.gp[,1]==0)   # false positives
          roc.grid[p,5] = sum(prob_tst[,2]<=cutoff & tst.gp[,1]==1)  # false negatives
          roc.grid[p,6] = roc.grid[p,2]/(roc.grid[p,2] + roc.grid[p,5])
          roc.grid[p,7] = roc.grid[p,4]/(roc.grid[p,3] + roc.grid[p,4])
        }
        ...
        write.table(cbind(sample.sizes[i], s, sigma,c.list[j],l,roc.area.tst,concordance.tst),
                    paste0('//10.8.8.51/lv0/Saumya/Datasets/gp_testing_retained_', min.sample.size,'_', max.sample.size, '.csv'), 
                    append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)      
      }
...
model <- read.csv(file="//10.8.8.51/lv0/Saumya/Datasets/gp_model_ensemble_retained.csv",head=TRUE,sep=",")

headers1 <- cbind("samplesize", "run", "sigma","cost","trainingerror","CVerror","SV_TotalObs",
                  "AUC","Concordance", "Test_AUC", "Test_Concordance")
write.table(headers1, paste0('//10.8.8.51/lv0/Saumya/Datasets/gp_model_results_ensemble_retained.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)
...
for (i in 1:nrow(model))
{
  ...
  for (s in 1:10)
  {
    ...
    gp.data.ones <- gp.data[gp.data[,2]==1,]
    index.ones.tra <- bigsample(1:nrow(gp.data.ones), size=0.5*sample.size, replace=F)
    tra.gp.ones <- gp.data.ones[ index.ones.tra,]
    tst.gp.ones <- gp.data.ones[-index.ones.tra,]
    ...
    gp.data.zeroes <- gp.data[gp.data[,2]!=1,]
    index.zeroes.tra <- bigsample(1:nrow(gp.data.zeroes), size=0.5*sample.size, replace=F)
    tra.gp.zeroes <- gp.data.zeroes[ index.zeroes.tra,]
    tst.gp.zeroes <- gp.data.zeroes[-index.zeroes.tra,]
    ...
    for (l in 1:20)
    {
      ...
      tst.gp <- rbind(tst.gp.ones, tst.gp.zeroes)[index.tst.gp, ]
      tst.gp<-tst.gp[c(-1)]
      gc()
      prob_tst<- predict(ksvm.model,tst.gp[c(-1)],type="probabilities")
    ...
    write.table(cbind(sample.size, s, sigma, cost, attr(ksvm.model,"error"), 
                ...
                paste0('//10.8.8.51/lv0/Saumya/Datasets/gp_model_results_ensemble_retained.csv'),
                append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)
    ...
    if (roc.area.tst >= test.auc & concordance.tst >= test.concordance)
    {
      print('-------------------- Saving Model -------------------------')
      print(ksvm.object)
      save(ksvm.model, file = paste0("//10.8.8.51/lv0/Saumya/Datasets/SVM_Models/GP/Retained/","model",i,".RData"))
