...
%MACRO IMPUTE(LIBNAME, SAMPLES, INPUTLIB, BRAND);
 ...
 %DO I=1 %TO &SAMPLECOUNT;
   ...
   PROC UNIVARIATE DATA = &LIBNAME..&BRAND._&SAMPLENAME._COMBINED;
	 VAR NUM_PLCC_TXNS_12MO_SLS;
   RUN;
...
PROC REG DATA=ON_EM_ALLCAMPAIGNS_BALANCED_3;
  MODEL RESPONDER =	NUM_TXNS_12MO_SLS ONSALE_QTY_12MO
					...
					EMAILS_CLICKED_ON EMAILS_VIEWED_ON PLCC_TXN_PCT
					/VIF;
  WHERE TRAINING=1;
RUN;
QUIT;
...
PROC LOGISTIC DATA=ON_EM_ALLCAMPAIGNS_BALANCED_3
  ...
  MODEL RESPONDER(EVENT='1')=   NUM_TXNS_12MO_SLS ONSALE_QTY_12MO
								...
								ON_PROMOTIONS_RECEIVED_12MO DAYS_LAST_PUR_ON
								AVG_UNT_RTL NET_SALES_ONL_PCT_12MO DISCOUNT_PCT_12MO
								EMAILS_CLICKED_ON CARD_STATUS 
								/* EMAILS_VIEWED_ON PLCC_TXN_PCT */ 
...
PROC SQL;
 SELECT COUNT(*) INTO: TRAINCOUNT FROM ON_EM_ALLCAMPAIGNS_COMBINED_4 WHERE TRAINING=1 AND ACTIVE_FLAG EQ 1;
QUIT;
...
PROC SORT DATA=ON_EM_ALLCAMPAIGNS_COMBINED_4
 ...
 BY DESCENDING POFFSET_1;
 WHERE TRAINING=1 AND ACTIVE_FLAG EQ 1;
RUN;
...
PROC SQL;
 SELECT COUNT(*) INTO: TESTCOUNT FROM ON_EM_ALLCAMPAIGNS_COMBINED_4 WHERE TRAINING=0  AND ACTIVE_FLAG EQ 1;
QUIT;
...
PROC SORT DATA=ON_EM_ALLCAMPAIGNS_COMBINED_4
 ...
 BY DESCENDING POFFSET_1;
 WHERE TRAINING=0  AND ACTIVE_FLAG EQ 1;
RUN;
