...
DATA DATALIB.ON_EM_ALLCAMPAIGNS_RESP_PROP_1;
 SET DATALIB.ON_EM_ALLCAMPAIGNS_RESP_PROP_1;
...
%LET SAMPLENAME = 261780;


DATA DATALIB.ON_&SAMPLENAME._COMBINED;
 SET DATALIB.ON_&SAMPLENAME._COMBINED;
...
DATA ON_&SAMPLENAME._COMBINED_1;
...
DATA ON_&SAMPLENAME._COMBINED_2;
SET ON_&SAMPLENAME._COMBINED_1;
...
DATA ON_&SAMPLENAME._COMBINED_3;
SET ON_&SAMPLENAME._COMBINED_2;
...
PROC MEANS DATA=ON_&SAMPLENAME._COMBINED_3 N NMISS;
VAR _NUMERIC_;
RUN;

DATA ON_&SAMPLENAME._COMBINED_3;
 SET ON_&SAMPLENAME._COMBINED_3;
 IF CARD_STATUS NOT IN (2,3,4) THEN DELETE;
RUN;

PROC LOGISTIC INMODEL=DATALIB.ON_EM_BALANCED_MODEL_1;
 SCORE DATA=ON_&SAMPLENAME._COMBINED_3 OUT=ON_&SAMPLENAME._COMBINED_4;
RUN;


DATA DATALIB.ON_&SAMPLENAME._COMBINED_SCORE;
 KEEP CAMPAIGN CUSTOMER_KEY CARD_STATUS NUM_TXNS_12MO_SLS 
	  ACTIVE_FLAG RESPONDER P_1 POFFSET_1;
 SET ON_&SAMPLENAME._COMBINED_4;
 IF P_1 NE 1;
...
PROC LOGISTIC DATA = DATALIB.ON_&SAMPLENAME._COMBINED_SCORE;
 MODEL RESPONDER (EVENT = '1') = RESP_PROB_1;
RUN;


PROC SQL;
 SELECT COUNT(*) INTO: COUNT FROM DATALIB.ON_&SAMPLENAME._COMBINED_SCORE;
QUIT;
 
%LET INTV = %EVAL(&COUNT/10);

PROC SORT DATA=DATALIB.ON_&SAMPLENAME._COMBINED_SCORE
		  OUT=DATALIB.ON_&SAMPLENAME._COMBINED_SCORE;
 BY DESCENDING RESP_PROB_1;
RUN;


DATA DATALIB.ON_&SAMPLENAME._COMBINED_SCORE;
 SET DATALIB.ON_&SAMPLENAME._COMBINED_SCORE;
...
PROC FREQ DATA=DATALIB.ON_&SAMPLENAME._COMBINED_SCORE;
...
PROC SQL;
 CREATE TABLE SUMMARY AS SELECT DISTINCT 
	    ...
	    FROM DATALIB.ON_&SAMPLENAME._COMBINED_SCORE
