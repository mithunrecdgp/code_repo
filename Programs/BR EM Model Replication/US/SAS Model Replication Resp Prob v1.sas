...
%CONVERTHIVE
(
SAMPLES=256164 257741 258567 259580 260420 261273,
BRAND=BR,
...
*256164 257741 258567 259580 260420 261273;
...
PROC TRANSPOSE DATA=DATALIB.BR_EM_ALLCAMPAIGNS_PCTL
			   OUT =DATALIB.BR_EM_ALLCAMPAIGNS_PCTL_T;
RUN;


DATA DATALIB.BR_EM_ALLCAMPAIGNS_PCTL_T;
 SET DATALIB.BR_EM_ALLCAMPAIGNS_PCTL_T;
 DROP _LABEL_;
...
PROC SORT DATA=DATALIB.BR_EM_ALLCAMPAIGNS_PCTL_T
          OUT=DATALIB.BR_EM_ALLCAMPAIGNS_PCTL_T;
 BY PERCENTILE_PTS;
RUN;

DATA BR_EM_ALLCAMPAIGNS_BALANCED_1;
 KEEP AVG_ORD_SZ_12MO_CP 
      ...
      NUM_TXNS_ONL_PCT_12MO
      ITEM_QTY_ONL_PCT_12MO; 
 SET DATALIB.BR_EM_ALLCAMPAIGNS_BALANCED;
RUN;
...
DATA BR_EM_ALLCAMPAIGNS_BALANCED_2;
SET BR_EM_ALLCAMPAIGNS_BALANCED_1;
...
DATA BR_EM_ALLCAMPAIGNS_BALANCED_3;
SET BR_EM_ALLCAMPAIGNS_BALANCED_2;
...
PROC EXPORT DATA=BR_EM_ALLCAMPAIGNS_BALANCED_3
			OUTFILE="Z:\TANUMOY\DATASETS\MODEL REPLICATION\BR_EM_ALLCAMPAIGNS_BALANCED_3.TXT"
            DBMS=DLM REPLACE;
     DELIMITER="|";
RUN;
...
PROC LOGISTIC DATA=BR_EM_ALLCAMPAIGNS_BALANCED_3
			  OUTEST=DATALIB.BR_EM_BALANCED_PARMS_1
              OUTMODEL=DATALIB.BR_EM_BALANCED_MODEL_1;
  CLASS CARD_STATUS (REF='4');
  MODEL RESPONDER(EVENT='1')=   NET_SALES_AMT_12MO_SLS ONSALE_QTY_12MO CARD_STATUS
								RESPONSE_RATE_12MO DIV_SHP_BR YEARS_ON_BOOKS
								BR_PROMOTIONS_RECEIVED_12MO DAYS_LAST_PUR_BR
								AVG_UNT_RTL NET_SALES_ONL_PCT_12MO DISCOUNT_PCT_12MO
								EMAILS_CLICKED_BR EMAILS_VIEWED_BR PLCC_TXN_PCT  
  ...
  WHERE TRAINING=1;
  OUTPUT OUT=BR_EM_BALANCED_FITS_1 P=P_FIXED XBETA=L_FIXED;
RUN;

DATA BR_EM_BALANCED_FITS_1;
 SET BR_EM_BALANCED_FITS_1;
...
PROC LOGISTIC INMODEL=DATALIB.BR_EM_BALANCED_MODEL_1;
 SCORE DATA=BR_EM_ALLCAMPAIGNS_BALANCED_3 OUT=BR_EM_BALANCED_PRED_1;
RUN;


DATA BR_EM_BALANCED_PRED_1;
 SET BR_EM_BALANCED_PRED_1;
 IF P_1 NE 1;
...
PROC SQL;
 SELECT COUNT(*) INTO: TRAINCOUNT FROM BR_EM_BALANCED_PRED_1 WHERE TRAINING=1;
QUIT;
 
%LET TRAININTV = %EVAL(&TRAINCOUNT/10);

PROC SORT DATA=BR_EM_BALANCED_PRED_1
		  OUT=BR_EM_BALANCED_PRED_TRAIN;
 BY DESCENDING POFFSET_1;
 WHERE TRAINING=1;
RUN;


DATA BR_EM_BALANCED_PRED_TRAIN;
 SET BR_EM_BALANCED_PRED_TRAIN;
...
PROC FREQ DATA=BR_EM_BALANCED_PRED_TRAIN;
...
PROC SQL;
 CREATE TABLE SUMMARY_TRAIN AS SELECT DISTINCT 
	    ...
	    FROM BR_EM_BALANCED_PRED_TRAIN
...
PROC SQL;
 SELECT COUNT(*) INTO: TESTCOUNT FROM BR_EM_BALANCED_PRED_1 WHERE TESTING=1;
QUIT;
 
%LET TESTINTV = %EVAL(&TESTCOUNT/10);

PROC SORT DATA=BR_EM_BALANCED_PRED_1
		  OUT=BR_EM_BALANCED_PRED_TEST;
 BY DESCENDING POFFSET_1;
 WHERE TESTING=1;
RUN;


DATA BR_EM_BALANCED_PRED_TEST;
 SET BR_EM_BALANCED_PRED_TEST;
...
PROC FREQ DATA=BR_EM_BALANCED_PRED_TEST;
 TABLE NEW_RESP_PROB_GRP * RESPONDER;
RUN;
...
DATA BR_EM_ALLCAMPAIGNS_COMBINED_1;
 KEEP AVG_ORD_SZ_12MO_CP 
	  AVG_ORD_SZ_12MO 
 	  CARD_STATUS
	  ONSALE_QTY_12MO 
      ...
      NUM_TXNS_ONL_PCT_12MO
      ITEM_QTY_ONL_PCT_12MO; 
 SET DATALIB.BR_EM_ALLCAMPAIGNS_COMBINED;
RUN;
...
DATA BR_EM_ALLCAMPAIGNS_COMBINED_2;
SET BR_EM_ALLCAMPAIGNS_COMBINED_1;
...
DATA BR_EM_ALLCAMPAIGNS_COMBINED_3;
SET BR_EM_ALLCAMPAIGNS_COMBINED_2;
...
PROC MEANS DATA=BR_EM_ALLCAMPAIGNS_COMBINED_3 N NMISS;
VAR _NUMERIC_;
RUN;

PROC LOGISTIC INMODEL=DATALIB.BR_EM_BALANCED_MODEL_1;
 SCORE DATA=BR_EM_ALLCAMPAIGNS_COMBINED_3 OUT=BR_EM_ALLCAMPAIGNS_COMBINED_4;
RUN;


DATA BR_EM_ALLCAMPAIGNS_COMBINED_4;
 KEEP CAMPAIGN TRAINING CUSTOMER_KEY CARD_STATUS NUM_TXNS_12MO_SLS ACTIVE_FLAG RESPONDER P_1 POFFSET_1;
 SET BR_EM_ALLCAMPAIGNS_COMBINED_4;
 IF P_1 NE 1;
 LOGIT_1=LOG(P_1) - LOG(1-P_1);
 POFFSET_1=LOGISTIC(LOGIT_1-OFFSET);
 LABEL P_1 = "RESP_PROB_1";
...
PROC SQL;
 SELECT COUNT(*) INTO: TRAINCOUNT FROM BR_EM_ALLCAMPAIGNS_COMBINED_4 WHERE TRAINING=1 AND ACTIVE_FLAG NE 1;
QUIT;
 
%LET TRAININTV = %EVAL(&TRAINCOUNT/10);

PROC SORT DATA=BR_EM_ALLCAMPAIGNS_COMBINED_4
		  OUT=BR_EM_COMBINED_PRED_TRAIN;
 BY DESCENDING POFFSET_1;
 WHERE TRAINING=1 AND ACTIVE_FLAG NE 1;
RUN;


DATA BR_EM_COMBINED_PRED_TRAIN;
 SET BR_EM_COMBINED_PRED_TRAIN;
 IF _N_ GE 1 + 0 * ROUND(&TRAININTV) THEN NEW_RESP_PROB_GRP=1;
 IF _N_ GE 1 + 1 * ROUND(&TRAININTV) THEN NEW_RESP_PROB_GRP=2;
 IF _N_ GE 1 + 2 * ROUND(&TRAININTV) THEN NEW_RESP_PROB_GRP=3;
 IF _N_ GE 1 + 3 * ROUND(&TRAININTV) THEN NEW_RESP_PROB_GRP=4;
...
PROC FREQ DATA=BR_EM_COMBINED_PRED_TRAIN;
 TABLE NEW_RESP_PROB_GRP * RESPONDER;
RUN;
...
PROC SQL;
 CREATE TABLE SUMMARY_TRAIN AS SELECT DISTINCT 
        NEW_RESP_PROB_GRP, COUNT(CUSTOMER_KEY) AS CUSTOMERS,
        SUM(RESPONDER) AS RESPONDERS
	    FROM BR_EM_COMBINED_PRED_TRAIN
		GROUP BY NEW_RESP_PROB_GRP ORDER BY NEW_RESP_PROB_GRP;
QUIT;
...
PROC SQL;
 SELECT COUNT(*) INTO: TESTCOUNT FROM BR_EM_ALLCAMPAIGNS_COMBINED_4 WHERE TRAINING=0  AND ACTIVE_FLAG NE 1;
QUIT;
 
%LET TESTINTV = %EVAL(&TESTCOUNT/10);

PROC SORT DATA=BR_EM_ALLCAMPAIGNS_COMBINED_4
		  OUT=BR_EM_COMBINED_PRED_TEST;
 BY DESCENDING POFFSET_1;
 WHERE TRAINING=0  AND ACTIVE_FLAG NE 1;
RUN;


DATA BR_EM_COMBINED_PRED_TEST;
 SET BR_EM_COMBINED_PRED_TEST;
 IF _N_ GE 1 + 0 * ROUND(&TESTINTV) THEN NEW_RESP_PROB_GRP=1;
 IF _N_ GE 1 + 1 * ROUND(&TESTINTV) THEN NEW_RESP_PROB_GRP=2;
 IF _N_ GE 1 + 2 * ROUND(&TESTINTV) THEN NEW_RESP_PROB_GRP=3;
 IF _N_ GE 1 + 3 * ROUND(&TESTINTV) THEN NEW_RESP_PROB_GRP=4;
...
PROC FREQ DATA=BR_EM_COMBINED_PRED_TEST;
 TABLE NEW_RESP_PROB_GRP * RESPONDER;
RUN;
...
PROC SQL;
 SELECT COUNT(*) INTO: TRAINCOUNT FROM BR_EM_ALLCAMPAIGNS_COMBINED_4 WHERE TRAINING=1;
QUIT;
...
PROC SQL;
 SELECT COUNT(*) INTO: TESTCOUNT FROM BR_EM_ALLCAMPAIGNS_COMBINED_4 WHERE TRAINING=0 ;
QUIT;
