...
%MACRO IMPORT(LIBNAME, SAMPLES, INPUTLIB);
 ...
 DATA TEMP1;  SET TEMP1;
  WHERE MEMNAME NE "QUEST_TEMP_EXPLAIN" AND INDEX(MEMNAME,"DATERANGE")=0
		  ...
		  AND  INDEX(MEMNAME,"BR")>0 AND INDEX(MEMNAME,"MAILED_SEGS")=0
 ...
 %DO I=1 %TO &SAMPLECOUNT;
   ...
   %LET BASEDS=BR_&SAMPLENAME._DM;
    ...
    PROC SORT DATA=&LIBNAME..&BASEDS OUT=&LIBNAME..BR_&SAMPLENAME._COMBINED;
	 BY CUSTOMER_KEY;
	RUN;
   ...
   %DO J=1 %TO &NUM_DATASETS;
	 ...
	 %IF &DSNAME NE &BASEDS %THEN %DO;
		...
		DATA &LIBNAME..BR_&SAMPLENAME._COMBINED;
		 MERGE &LIBNAME..BR_&SAMPLENAME._COMBINED (IN=A)
			   &LIBNAME..&DSNAME (IN=B);
  		 BY CUSTOMER_KEY;
		 IF A=1;
		RUN;
   ...
   DATA &LIBNAME..BR_&SAMPLENAME._COMBINED;
    SET &LIBNAME..BR_&SAMPLENAME._COMBINED;
   ...
   DATA &LIBNAME..BR_&SAMPLENAME._COMBINED(DROP=Z);                                                    
    SET &LIBNAME..BR_&SAMPLENAME._COMBINED;                                                            
...
%IMPORT(LIBNAME=DATALIB, INPUTLIB=TA4U1R9, SAMPLES = 234099);

* 234099 236436 239342 240818 241358 242052 242140 242688;
...
%MACRO APPEND(LIBNAME, SAMPLES, INPUTLIB);
 ...
 %DO I=1 %TO &SAMPLECOUNT;
   ...
   %IF &I=1 %THEN %DO;
    DATA &LIBNAME..BR_ALLCAMPAIGNS_COMBINED;
	 SET &LIBNAME..BR_&SAMPLENAME._COMBINED;
	RUN;
   %END;

   %IF &I>1 %THEN %DO;
    PROC APPEND BASE=&LIBNAME..BR_ALLCAMPAIGNS_COMBINED
				DATA=&LIBNAME..BR_&SAMPLENAME._COMBINED FORCE;
	RUN;
   %END;
...
%APPEND(LIBNAME=DATALIB, INPUTLIB=TA4U1R9, SAMPLES= 234099 236436 239342 240818 241358 242052 242140 242688);
...
%MACRO CHECK(LIBNAME, SAMPLES, INPUTLIB);
 ...
 %DO I=1 %TO &SAMPLECOUNT;
   ...
   PROC SQL UNDO_POLICY=NONE;
    SELECT DISTINCT START_DT, END_DT FROM &LIBNAME..BR_&SAMPLENAME._DM;
   QUIT;
   
   PROC FREQ DATA=&LIBNAME..BR_&SAMPLENAME._COMBINED;
...
%CHECK(LIBNAME=DATALIB, INPUTLIB=TA4U1R9, SAMPLES= 234099 236436 239342 240818 241358 242052 242140 242688);
...
%MACRO COUNTS(LIBNAME, SAMPLES, INPUTLIB);
 ...
 %DO I=1 %TO &SAMPLECOUNT;
   ...
   PROC SQL UNDO_POLICY=NONE;
	...
	FROM &LIBNAME..BR_&SAMPLENAME._DM;
 
	SELECT DISTINCT COUNT(CUSTOMER_KEY) AS RECORDS,
		   COUNT(DISTINCT CUSTOMER_KEY) AS DIST_CUST 
	FROM &LIBNAME..BR_&SAMPLENAME._COMBINED;
...
%COUNTS(LIBNAME=DATALIB, INPUTLIB=TA4U1R9, SAMPLES=234099 236436 239342 240818 241358 242052 242140 242688);

*234099 236436 239342 240818 241358 242052 242140 242688;


DATA DATALIB.BR_ALLCAMPAIGNS_COMBINED;
 SET DATALIB.BR_ALLCAMPAIGNS_COMBINED;
 TREATMENT_GROUP=1;
...
PROC SQL;
 SELECT COUNT(*) INTO: RESPONDERS FROM DATALIB.BR_ALLCAMPAIGNS_COMBINED WHERE RESPONDER=1;
 SELECT COUNT(*) INTO: NONRESPONDERS FROM DATALIB.BR_ALLCAMPAIGNS_COMBINED WHERE RESPONDER=0;
QUIT;
...
DATA DATALIB.BR_ALLCAMPAIGNS_NONRESPONDERS;
 SET DATALIB.BR_ALLCAMPAIGNS_COMBINED;
 WHERE RESPONDER=0;
 RANDOM_NO=RANUNI(&SEED);
RUN;

PROC SORT DATA=DATALIB.BR_ALLCAMPAIGNS_NONRESPONDERS 
		  OUT=BR_ALLCAMPAIGNS_NONRESPONDERS;
 BY RANDOM_NO;
RUN;

DATA BR_ALLCAMPAIGNS_NONRESPONDERS;
 SET BR_ALLCAMPAIGNS_NONRESPONDERS;
 IF _N_<=ROUND(&RESPONDERS);
 DROP RANDOM_NO;
RUN;

DATA DATALIB.BR_ALLCAMPAIGNS_BALANCED;
 SET DATALIB.BR_ALLCAMPAIGNS_RESPONDERS;
RUN;

PROC APPEND BASE=DATALIB.BR_ALLCAMPAIGNS_BALANCED
            DATA=BR_ALLCAMPAIGNS_NONRESPONDERS FORCE;
RUN;

PROC FREQ DATA=DATALIB.BR_ALLCAMPAIGNS_BALANCED;
 TABLE RESPONDER;
RUN;
...
PROC SQL;
 SELECT COUNT(*) INTO: RESPONDERS_BAL FROM DATALIB.BR_ALLCAMPAIGNS_BALANCED WHERE RESPONDER=1;
 SELECT COUNT(*) INTO: NONRESPONDERS_BAL FROM DATALIB.BR_ALLCAMPAIGNS_BALANCED WHERE RESPONDER=0;
QUIT;
...
DATA DATALIB.BR_ALLCAMPAIGNS_RESP_PROP;
...
DATA DATALIB.BR_ALLCAMPAIGNS_RESP_PROP;
 SET DATALIB.BR_ALLCAMPAIGNS_RESP_PROP;
...
DATA DATALIB.BR_ALLCAMPAIGNS_BALANCED;
 SET DATALIB.BR_ALLCAMPAIGNS_BALANCED;
...
PROC UNIVARIATE DATA=DATALIB.BR_ALLCAMPAIGNS_COMBINED;
 VAR AVG_ORD_SZ_12MO_CP  AVG_ORD_SZ_12MO ONSALE_QTY_12MO
     ...
     DIV_SHP_BR BR_PROMOTIONS_RECEIVED_12MO ONSALE_QTY_6MO
	 DAYS_LAST_PUR_BR DAYS_LAST_PUR_CP PLCC_TXN_PCT NUM_PLCC_TXNS_12MO_SLS;
     
OUTPUT OUT=DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL
	   PCTLPRE = AVG_ORD_SZ_12MO_CP_ AVG_ORD_SZ_12MO_ ONSALE_QTY_12MO_ 
				 ...
				 UNITS_PER_TXN_12MO_SLS_ NUM_TXNS_12MO_SLS_ NUM_TXNS_12MO_RTN_
     			 DIV_SHP_BR_ BR_PROMOTIONS_RECEIVED_12MO_ ONSALE_QTY_6MO_
				 DAYS_LAST_PUR_BR_ DAYS_LAST_PUR_CP_ PLCC_TXN_PCT_ NUM_PLCC_TXNS_12MO_SLS_
	   PCTLPTS= 0 0.3 0.5 1 50 99 99.5 99.7 100;
RUN;


PROC TRANSPOSE DATA=DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL
			   OUT =DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL_T;
RUN;


DATA DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL_T;
 SET DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL_T;
 DROP _LABEL_;
 WHERE INDEX(_NAME_,"_0_5") > 0 OR INDEX(_NAME_,"_99_5") > 0;
 IF INDEX(_NAME_,"_0_5") > 0 THEN PERCENTILE_PTS = 0.5;
 IF INDEX(_NAME_,"_99_5") > 0 THEN PERCENTILE_PTS = 99.5;
RUN;

PROC SORT DATA=DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL_T
          OUT=DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL_T;
 BY PERCENTILE_PTS;
RUN;


DATA BR_ALLCAMPAIGNS_BALANCED_1;
 KEEP NET_SALES_AMT AVG_ORD_SZ_12MO_CP AVG_ORD_SZ_12MO
	  ONSALE_QTY_12MO NET_SALES_AMT_12MO_RTN NET_SALES_AMT_12MO_SLS 
	  AVG_UNT_RTL_CP AVG_UNT_RTL DISCOUNT_PCT_12MO YEARS_ON_BOOKS 
	  UNITS_PER_TXN_12MO_SLS NUM_TXNS_12MO_SLS  NUM_TXNS_12MO_RTN
      DIV_SHP_BR BR_PROMOTIONS_RECEIVED_12MO ONSALE_QTY_6MO TESTING
	  TREATMENT_GROUP CUSTOMER_KEY RESPONDER TRAINING CAMPAIGN OFFSET  
	  WT DAYS_LAST_PUR_BR DAYS_LAST_PUR_CP LENGTH_PROMO
	  PLCC_TXN_PCT NUM_PLCC_TXNS_12MO_SLS CAMPAIGN_CODE RESPONSE_RATE_12MO
	  BR_PROMOTIONS_RESPONDED_12MO BASIC_FLAG SILVER_FLAG SISTER_FLAG 
	  CARD_STATUS NONCARD_FLAG; 
 SET DATALIB.BR_ALLCAMPAIGNS_BALANCED;
...
SET DATALIB.BR_ALLCAMPAIGNS_COMBINED_PCTL;
 ...
 CALL SYMPUT ("DIV_SHP_BR_LC", DIV_SHP_BR_0);
 CALL SYMPUT ("DIV_SHP_BR_50", DIV_SHP_BR_50);
 CALL SYMPUT ("DIV_SHP_BR_UC", DIV_SHP_BR_100);
...
DATA BR_ALLCAMPAIGNS_BALANCED_2;
SET BR_ALLCAMPAIGNS_BALANCED_1;
...
IF DIV_SHP_BR > &DIV_SHP_BR_UC THEN DIV_SHP_BR = &DIV_SHP_BR_UC;
IF BR_PROMOTIONS_RECEIVED_12MO > &BR_PROMOTIONS_RECEIVED_12MO_UC THEN BR_PROMOTIONS_RECEIVED_12MO = &BR_PROMOTIONS_RECEIVED_12MO_UC;
...
IF DAYS_LAST_PUR_BR > &DAYS_LAST_PUR_BR_UC THEN DAYS_LAST_PUR_BR = &DAYS_LAST_PUR_BR_UC;
...
IF DIV_SHP_BR < &DIV_SHP_BR_LC THEN DIV_SHP_BR = &DIV_SHP_BR_LC;
IF BR_PROMOTIONS_RECEIVED_12MO < &BR_PROMOTIONS_RECEIVED_12MO_LC THEN BR_PROMOTIONS_RECEIVED_12MO = &BR_PROMOTIONS_RECEIVED_12MO_LC;
...
IF DAYS_LAST_PUR_BR < &DAYS_LAST_PUR_BR_LC THEN DAYS_LAST_PUR_BR = &DAYS_LAST_PUR_BR_LC;
...
DATA BR_ALLCAMPAIGNS_BALANCED_3;
SET BR_ALLCAMPAIGNS_BALANCED_2;
...
PROC EXPORT DATA=BR_ALLCAMPAIGNS_BALANCED_3
			OUTFILE="Z:\TANUMOY\DATASETS\MODEL REPLICATION\BR_ALLCAMPAIGNS_BALANCED_3.TXT"
            DBMS=DLM REPLACE;
     DELIMITER="|";
RUN;
...
PROC REG DATA=BR_ALLCAMPAIGNS_BALANCED_3;
 MODEL  RESPONDER   =	NUM_TXNS_12MO_SLS
						DIV_SHP_BR
						YEARS_ON_BOOKS
						RESPONSE_RATE_12MO
...
PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_3
              OUTEST=DATALIB.BR_ALLCAMPAIGNS_BALANCED_PARMS_1
              OUTMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_1;
 MODEL   RESPONDER(REF="0")  =	NUM_TXNS_12MO_SLS			
								DIV_SHP_BR			
								YEARS_ON_BOOKS			
								AVG_UNT_RTL			
								BR_PROMOTIONS_RECEIVED_12MO			
 ...
 OUTPUT OUT=BR_ALLCAMPAIGNS_BALANCED_FITS_1 PREDICTED=P_FIXED XBETA=L_FIXED;
 WHERE TRAINING=1;
RUN;


PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_3
              OUTEST=DATALIB.BR_ALLCAMPAIGNS_BALANCED_PARMS_2
              OUTMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_2;
 ...
 OUTPUT OUT=BR_ALLCAMPAIGNS_BALANCED_FITS_2 PREDICTED=P_FIXED XBETA=L_FIXED;
 WHERE TRAINING=1;
RUN;


PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_3
              OUTEST=DATALIB.BR_ALLCAMPAIGNS_BALANCED_PARMS_3
              OUTMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_3;
 ...
 OUTPUT OUT=BR_ALLCAMPAIGNS_BALANCED_FITS_3 PREDICTED=P_FIXED XBETA=L_FIXED;
 WHERE TRAINING=1 AND CARD_STATUS NE 4;
RUN;


PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_3
              OUTEST=DATALIB.BR_ALLCAMPAIGNS_BALANCED_PARMS_4
              OUTMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_4;
 MODEL   RESPONDER(REF="0")  =	NUM_TXNS_12MO_SLS
								DIV_SHP_BR
								YEARS_ON_BOOKS
								RESPONSE_RATE_12MO
 ...
 OUTPUT OUT=BR_ALLCAMPAIGNS_BALANCED_FITS_4 PREDICTED=P_FIXED XBETA=L_FIXED;
 WHERE TRAINING=1 AND CARD_STATUS EQ 4;
RUN;


PROC GLIMMIX DATA=BR_ALLCAMPAIGNS_BALANCED_3 OUTDESIGN = TEMP METHOD=MSPL;
 CLASS CARD_STATUS;
 ...
 OUTPUT OUT = BR_ALLCAMPAIGNS_BALANCED_FITS_5 PRED( BLUP ILINK) = P_MIXED
											  PRED( NOBLUP ILINK) = P_FIXED;
 ID CAMPAIGN CUSTOMER_KEY RESPONDER _XBETA_ _ZGAMMA_;
 WHERE TRAINING=1;
 ODS OUTPUT TESTS3 = BR_ALLCAMPAIGNS_TYPE3_TESTS
			COVPARMS = BR_ALLCAMPAIGNS_COVPARMS 
			PARAMETERESTIMATES = BR_ALLCAMPAIGNS_PARAMEST;
RUN;

*ADJUSTING THE PROBABILITIES DUE TO BALANCING USING THE OFFSET;
 
DATA BR_ALLCAMPAIGNS_BALANCED_FITS_1;
 SET BR_ALLCAMPAIGNS_BALANCED_FITS_1;
...
DATA BR_ALLCAMPAIGNS_BALANCED_FITS_2;
 SET BR_ALLCAMPAIGNS_BALANCED_FITS_2;
 P_1=EXP(L_FIXED)/(1+EXP(L_FIXED));
 POFFSET=LOGISTIC(L_FIXED-OFFSET);
 LABEL L_FIXED = "LOGIT";
 DROP P_FIXED;
...
DATA BR_ALLCAMPAIGNS_BALANCED_FITS_3;
 SET BR_ALLCAMPAIGNS_BALANCED_FITS_3;
 P_1=EXP(L_FIXED)/(1+EXP(L_FIXED));
 POFFSET=LOGISTIC(L_FIXED-OFFSET);
 LABEL L_FIXED = "LOGIT";
 DROP P_FIXED;
...
DATA BR_ALLCAMPAIGNS_BALANCED_FITS_4;
 SET BR_ALLCAMPAIGNS_BALANCED_FITS_4;
 P_1=EXP(L_FIXED)/(1+EXP(L_FIXED));
 POFFSET=LOGISTIC(L_FIXED-OFFSET);
 LABEL L_FIXED = "LOGIT";
 DROP P_FIXED;
...
PROC APPEND BASE=BR_ALLCAMPAIGNS_BALANCED_FITS_3
			DATA=BR_ALLCAMPAIGNS_BALANCED_FITS_4
			FORCE;
RUN;

PROC SQL;
 DROP TABLE BR_ALLCAMPAIGNS_BALANCED_FITS_4;
QUIT;


PROC SQL UNDO_POLICY = NONE;
 CREATE TABLE BR_ALLCAMPAIGNS_BALANCED_FITS_4 AS SELECT T1.*, T2._XBETA_, T2._ZGAMMA_, T2.P_FIXED, T2.P_MIXED 
 		FROM BR_ALLCAMPAIGNS_BALANCED_3 T1,
			 BR_ALLCAMPAIGNS_BALANCED_FITS_5 T2
...
PROC SQL;
 DROP TABLE BR_ALLCAMPAIGNS_BALANCED_FITS_5;
QUIT;



PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_FITS_1;
 MODEL RESPONDER(EVENT='1') = POFFSET;
RUN;

PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_FITS_2;
 MODEL RESPONDER(EVENT='1') = POFFSET;
RUN;

PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_FITS_3;
 MODEL RESPONDER(EVENT='1') = POFFSET;
RUN;

PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_FITS_4;
 MODEL RESPONDER(EVENT='1') = POFFSET_FIXED / NOINT;
RUN;

PROC LOGISTIC DATA=BR_ALLCAMPAIGNS_BALANCED_FITS_4;
 MODEL RESPONDER(EVENT='1') = POFFSET_MIXED / NOINT;
RUN;


PROC LOGISTIC INMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_1;
 SCORE DATA= BR_ALLCAMPAIGNS_BALANCED_3 OUT=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_1;
RUN;


DATA DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_1;
 SET DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_1;
 IF P_1 NE 1;
...
PROC LOGISTIC INMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_2;
 SCORE DATA= BR_ALLCAMPAIGNS_BALANCED_3 OUT=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_2;
RUN;


DATA DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_2;
 SET DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_2;
 IF P_1 NE 1;
 LOGIT=LOG(P_1) - LOG(1-P_1);
 POFFSET=LOGISTIC(LOGIT-OFFSET);
 DROP P_0;
...
DATA BR_ALLCAMPAIGNS_BALANCED_3_TEMP;
 SET BR_ALLCAMPAIGNS_BALANCED_3;
 WHERE CARD_STATUS NE 4;
RUN;

PROC LOGISTIC INMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_3;
 SCORE DATA= BR_ALLCAMPAIGNS_BALANCED_3_TEMP OUT=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_3;
RUN;


DATA DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_3;
 SET DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_3;
 IF P_1 NE 1;
 LOGIT=LOG(P_1) - LOG(1-P_1);
 POFFSET=LOGISTIC(LOGIT-OFFSET);
 DROP P_0;
...
PROC LOGISTIC INMODEL=DATALIB.BR_ALLCAMPAIGNS_BALANCED_MODEL_4;
 SCORE DATA= BR_ALLCAMPAIGNS_BALANCED_3_TEMP OUT=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4;
RUN;


DATA DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4;
 SET DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4;
 IF P_1 NE 1;
 LOGIT=LOG(P_1) - LOG(1-P_1);
 POFFSET=LOGISTIC(LOGIT-OFFSET);
 DROP P_0;
...
PROC APPEND BASE=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_3
			DATA=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4
			FORCE;
RUN;

PROC SQL;
 DROP TABLE DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4;
QUIT;


DATA BR_ALLCAMPAIGNS_BALANCED_4_1;
 SET BR_ALLCAMPAIGNS_BALANCED_3;
 WHERE TRAINING = 1 ;
RUN;

DATA BR_ALLCAMPAIGNS_BALANCED_4_2;
 SET BR_ALLCAMPAIGNS_BALANCED_3;
 RESPONDER = . ;
RUN;

DATA BR_ALLCAMPAIGNS_BALANCED_4;
 SET BR_ALLCAMPAIGNS_BALANCED_4_1
 	 BR_ALLCAMPAIGNS_BALANCED_4_2;
RUN;

PROC SQL;
 DROP TABLE BR_ALLCAMPAIGNS_BALANCED_4_1;
 DROP TABLE BR_ALLCAMPAIGNS_BALANCED_4_2;
QUIT;

PROC GLIMMIX DATA=BR_ALLCAMPAIGNS_BALANCED_4 METHOD=MSPL;
 CLASS CARD_STATUS;
 MODEL RESPONDER 
				 = NUM_TXNS_12MO_SLS  DIV_SHP_BR PLCC_TXN_PCT
				   BR_PROMOTIONS_RECEIVED_12MO DAYS_LAST_PUR_BR
				   AVG_ORD_SZ_12MO ONSALE_QTY_12MO RESPONSE_RATE_12MO
				   / SOLUTION DISTRIBUTION=BINOMIAL LINK=LOGIT OFFSET=OFFSET;
 RANDOM INTERCEPT/ SUBJECT = CARD_STATUS;
 OUTPUT OUT = DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4 PRED( BLUP ILINK) = P_MIXED
											  		  PRED( NOBLUP ILINK) = P_FIXED;
...
PROC SQL UNDO_POLICY=NONE;
 CREATE TABLE DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4 AS SELECT T1.*, T2.RESPONDER, T2.TRAINING, T2.NET_SALES_AMT
		FROM DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_4 T1,
			 DATALIB.BR_ALLCAMPAIGNS_BALANCED T2
 WHERE T1.CAMPAIGN = T2.CAMPAIGN AND T1.CUSTOMER_KEY = T2.CUSTOMER_KEY
 ORDER BY T1.CAMPAIGN, T1.CUSTOMER_KEY;
QUIT;
...
%DO I=&STARTINDEX %TO &NO_MODELS;
	...
	PROC SQL;
	 SELECT COUNT(*) INTO: TRAINCOUNT FROM DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_&I WHERE TRAINING=1;
	QUIT;
	 
	%LET TRAININTV = %EVAL(&TRAINCOUNT/10);

	PROC SORT DATA=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_&I
			  OUT=BR_ALLCAMPAIGNS_BALANCED_RESP_1&I;
	 BY DESCENDING POFFSET DESCENDING NET_SALES_AMT;
	 WHERE TRAINING=1;
	RUN;


	DATA BR_ALLCAMPAIGNS_BALANCED_RESP_1&I;
	 SET BR_ALLCAMPAIGNS_BALANCED_RESP_1&I;
	...
	PROC FREQ DATA=BR_ALLCAMPAIGNS_BALANCED_RESP_1&I;
	...
	PROC SQL;
	 CREATE TABLE TEST_1&I AS SELECT DISTINCT NEW_RESP_PROB_GRP, COUNT(CUSTOMER_KEY) AS CUSTOMERS,
		    ...
		    FROM BR_ALLCAMPAIGNS_BALANCED_RESP_1&I
	...
	PROC SQL;
	 SELECT COUNT(*) INTO: TESTCOUNT FROM DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_&I WHERE TRAINING=0;
	QUIT;
	 
	%LET TESTINTV = %EVAL(&TESTCOUNT/10);

	PROC SORT DATA=DATALIB.BR_ALLCAMPAIGNS_BALANCED_RESP_&I
			  OUT=BR_ALLCAMPAIGNS_BALANCED_RESP_2&I;
	 BY DESCENDING POFFSET DESCENDING NET_SALES_AMT;
	 WHERE TRAINING=0;
	RUN;


	DATA BR_ALLCAMPAIGNS_BALANCED_RESP_2&I;
	 SET BR_ALLCAMPAIGNS_BALANCED_RESP_2&I;
	...
	PROC FREQ DATA=BR_ALLCAMPAIGNS_BALANCED_RESP_2&I;
	 TABLE NEW_RESP_PROB_GRP * RESPONDER;
	RUN;
	...
	PROC SQL;
	 CREATE TABLE TEST_2&I AS SELECT DISTINCT NEW_RESP_PROB_GRP, COUNT(CUSTOMER_KEY) AS CUSTOMERS,
	 									   SUM(RESPONDER) AS RESPONDERS, SUM(NET_SALES_AMT) AS NET_SALES
		    FROM BR_ALLCAMPAIGNS_BALANCED_RESP_2&I
			GROUP BY NEW_RESP_PROB_GRP ORDER BY NEW_RESP_PROB_GRP;
	QUIT;
