...
load("//10.8.8.51/lv0/Tanumoy/RData/ON Response Model.RData")

ds3<-read.table(file= "//10.8.8.51/lv0/Tanumoy/Datasets/Model Replication/ON_ALLCAMPAIGNS_BALANCED_3.txt",header=T,sep="|")
...
save.image("//10.8.8.51/lv0/Tanumoy/RData/ON Response Model.RData")
           ...
           append.results, startindex)
  {
      ...
      
    {
      ...
      write.table(coef.temp,
                  file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients Temp.TXT",
                  sep="|",row.names= FALSE,col.names= TRUE)
      coef.temp <- read.table(file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients Temp.TXT",sep="|",head=TRUE)
      file.remove("//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients Temp.TXT")    
    ...
    if (append.results==0)
    {
      ...
      write.table(coef.grid,
                  file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients.TXT",
                  sep="|",row.names= FALSE,col.names= TRUE)
      
      write.table(diagnostics.grid,
                  file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Diagnostics.TXT",
                  sep="|",row.names= FALSE,col.names= TRUE)
    ...
    if (append.results==1)
    {
      if (file.exists("//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients.txt"))
      {
        coef.grid <- read.table(file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients.TXT",sep="|",head=TRUE)
      ...
      {
        print("//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients.TXT does not exist.")
      }
    }
    
    if (append.results==1)
    {
      if (file.exists("//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Diagnostics.TXT"))
      {
        diagnostics.grid <- read.table(file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Diagnostics.TXT",sep="|",head=TRUE)
      ...
      {
        print("//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Diagnostics.TXT does not exist.")
      }
    }
...
logit.reg(dataset=df1, iterations=10,  trainpct=0.05, append.results=0,
          roc.graph=0, lift.graph=0, gains.graph=0, startindex=1)
...
resultsarray <- read.table(file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Diagnostics.TXT",sep="|",head=TRUE)

pdf(file="//10.8.8.51/lv0/Tanumoy/Graphs/ON MODEL ACCURACY LOGISTIC REGRESSION.PDF",height=9, width=16,bg="white")
par(mfrow=c(1,1))
...
coefarray <- read.table(file="//10.8.8.51/lv0/Tanumoy/RData/ON Response Model Coefficients.TXT",sep="|",head=TRUE)
coefloop=seq(1, ncol(coefarray) - 2)

pdf(file="//10.8.8.51/lv0/Tanumoy/Graphs/ON COEFFICIENT STABILITY LOGISTIC REGRESSION.PDF",height=9, width=16,bg="white")
par(mfrow=c(1,1))
